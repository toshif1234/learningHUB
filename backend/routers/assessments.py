...
async def submit_assessment(
    ...
    url = f"http://localhost:8000/assessments/attempts/{attempt_id}/certificate?signature={sig}" if is_passed else None
...
async def get_attempt_detail(
    ...
    url = f"http://localhost:8000/assessments/attempts/{attempt.id}/certificate?signature={sig}" if attempt.is_passed else None
...
async def get_passed_attempt(
    ...
    return {
        ...
        "signature": sig,
        "certificate_url": f"http://localhost:8000/assessments/attempts/{attempt.id}/certificate?signature={sig}"
    }
