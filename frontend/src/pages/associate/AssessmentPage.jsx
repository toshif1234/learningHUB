...
import { Award, Clock, ArrowLeft, ArrowRight, CheckSquare, Square, Check } from 'lucide-react';
...
export default function AssessmentPage() {
    ...
    const startQuiz = async () => {
      try {
        ...
        if (data.time_limit_minutes) {
          ...
          const deadlineTime = startedAt.getTime() + data.time_limit_minutes * 60 * 1000;
          const diffSeconds = Math.max(0, Math.floor((deadlineTime - Date.now()) / 1000));
  ...
  }, [timeLimit, secondsLeft, loading, submitting]);
  ...
  return (
    <div id="assessment-page-container" className="space-y-6 max-w-3xl mx-auto animate-fade-in pb-12">
      ...
      <div className="glass-card p-5 border border-dark-800 flex justify-between items-center flex-wrap gap-4">
        <div>
          <span className="text-xs font-semibold text-dark-400 uppercase tracking-widest">Ongoing Assessment</span>
          <h2 className="text-lg font-bold text-white mt-1">Question {activeQIdx + 1} of {questions.length}</h2>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="md:col-span-1 glass-card p-4 border border-dark-800 h-fit space-y-4">
          <h3 className="font-bold text-xs text-dark-400 uppercase tracking-widest text-center">Navigation</h3>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-dark-500 font-semibold">{activeQuestion.marks} Mark(s)</span>
              </div>
            ...
            <div className="space-y-3 pt-2">
              {activeQuestion.options.map((opt) => {
                ...
                return (
                  <div
                    key={opt.id}
                    onClick={() => handleSelectOption(activeQuestion.id, opt.id, activeQuestion.question_type)}
                  ...
                  >
                    <div className="flex-shrink-0 text-primary-400">
                      {isMulti ? (
                        ...
                        <div className={`w-5 h-5 rounded-full border flex items-center justify-center ${isSelected ? 'border-primary-500 bg-primary-500/20' : 'border-dark-600'}`}>
                          {isSelected && <div className="w-2 h-2 rounded-full bg-primary-400"></div>}
                        </div>
                      )}
                    </div>
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              type="button"
              onClick={() => { setConfirmOpen(false); performSubmit(); }}
              disabled={submitting}
      ...
      >
        <div className="space-y-3">
          <p className="text-sm text-dark-300">
            Are you sure you want to submit your answers? Please verify that you have answered all questions.
          </p>
